...
【最優先ルール：ユーザーのリクエストに従う】
「今日の気分・やりたいこと」欄にトピックや方向性の指定がある場合（例：「AI関連の企画が欲しい」「料理系で」「プログラミング初心者向け」）、
その指定を必ず企画のテーマに反映すること。プロフィールの要素はそのテーマをクリエイター独自の視点で差別化するために使う。
...
- 「AIを使って〜」「〜ツール紹介」「〜おすすめ5選」という薄い軸だけで企画を組み立てること（ただしユーザーがAI関連を明示的に要求した場合は、AIをテーマにしつつクリエイターの独自視点を必ず加えること）
...
【今日の気分・やりたいこと】
${mood}
（気分だけでなく「〇〇系の企画が欲しい」「〇〇について作りたい」などのトピック指定もここに入る。指定がある場合は必ずその方向で企画を作ること）
${profile.avoid ? `\n【絶対に含めないこと】\n${profile.avoid}` : ""}
